...
commands. Blocking gates such as `dp goal lint`, future `dp evidence lint`, hooks, CI, and
verification judgments must be deterministic, local, and free of model calls.

Generated artifacts must pass deterministic lint before they can be treated as ready.

## Consequences

- The first GoalContract slice implements deterministic lint before any synthesis command exists.
