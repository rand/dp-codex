...
1. Core engine (`/dp/core/`): parsing, validation, review, verify, decomposition, progress, goal lint, goal state, goal emission, evidence lint, loop-ledger scheduling, campaign manifest recovery, and campaign scaffolding.
...
13. `/dp/core/campaign_init.py`: conservative primary-spec campaign scaffolding.
14. `/dp/core/events.py`: JSONL event helpers.
15. `/dp/enforcement/engine.py`: executes policy-selected checks and bypass audit logging.
